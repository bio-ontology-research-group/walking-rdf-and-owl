...
new File("../data/goa_mouse.gaf").splitEachLine("\t") { line ->
  def id = line[1]
  uniprotids.add(id)
}

new File("../data/10090.protein.aliases.v10.txt").splitEachLine("\t") { line ->
  def stringid = line[0]
  def val = line[1]
  def type = line[2]
...
new File("../data/10090.protein.actions.v10.txt").splitEachLine("\t") { line ->
  if (line[0].startsWith("10090")) {
    def id1 = line[0]
    def id2 = line[1]
    def type = line[2]
